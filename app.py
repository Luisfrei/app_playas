    ...
    def get_marine_data(lat: float, lon: float):
        ...
        - wave_period (s)
        - wave_direction (°)
        - sea_level_height_msl (m, incluye la marea)
        """
    ...
    try:
        ...
        sea_level = (
            marine_hourly.get("sea_level_height")
            or marine_hourly.get("sea_level")
            or marine_hourly.get("water_level")
            or marine_hourly.get("tide_height")
            or []
        )
        sea_level_available = bool(sea_level)

        if not sea_level_available:
            st.info("No hay datos de marea disponibles desde la API marina. La tabla mostrará estado de oleaje pero no altura de marea.")
